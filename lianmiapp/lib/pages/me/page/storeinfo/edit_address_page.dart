...
class _AddressPageState extends State<AddressPage> {
  ...
  Widget build(BuildContext context) {
    if (isValidString(_ctrlAddress.text) == false) {
      StoreInfo? store =
          Provider.of<StoreInfoViewModel>(context, listen: false).store;
      _ctrlAddress = TextEditingController(text: store!.address);
    }
    return Theme(
        data: ThemeData(
        ...
        child: Scaffold(
            appBar: AppBar(
              ...
              backgroundColor: Colors.white,
              title: Text('商户地址', style: TextStyle(color: Color(0xff333333))),
              centerTitle: true,
              actions: [
                TextButton(
                  onPressed: () {
  ...
  void _confirm() {
    ...
    if (isValidString(_ctrlAddress.text) == false) {
      HubView.showToast('请输入地址');
      return;
    }
    HubView.showLoading();
    UserMod.updateStoreInfo(address: _ctrlAddress.text).then((value) {
      Provider.of<StoreInfoViewModel>(context, listen: false).updateStoreInfo();
      Future.delayed(Duration(seconds: 1), () {
        HubView.dismiss();
        HubView.showToastAfterLoadingHubDismiss('修改地址成功');
        AppNavigator.goBack(context);
      });
    }).catchError((err) {
      HubView.dismiss();
      HubView.showToastAfterLoadingHubDismiss('修改地址失败:$err');
    });
  }
}
