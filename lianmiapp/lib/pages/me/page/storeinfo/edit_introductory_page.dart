...
class IntroductoryPage extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _IntroductoryPageState();
  }
}

class _IntroductoryPageState extends State<IntroductoryPage> {
  TextEditingController _ctrlIntroductory = TextEditingController();
  ...
  Widget build(BuildContext context) {
    if (isValidString(_ctrlIntroductory.text) == false) {
      StoreInfo? store =
          Provider.of<StoreInfoViewModel>(context, listen: false).store;
      _ctrlIntroductory = TextEditingController(text: store!.introductory);
    }
    return Theme(
        data: ThemeData(
        ...
        child: Scaffold(
            appBar: AppBar(
              ...
              backgroundColor: Colors.white,
              title: Text('商户简介', style: TextStyle(color: Color(0xff333333))),
              centerTitle: true,
              actions: [
                TextButton(
                  onPressed: () {
  ...
  void _confirm() {
    ...
    HubView.showLoading();
    UserMod.updateStoreInfo(introductory: _ctrlIntroductory.text).then((value) {
      Provider.of<StoreInfoViewModel>(context, listen: false).updateStoreInfo();
      Future.delayed(Duration(seconds: 1), () {
        HubView.dismiss();
        HubView.showToastAfterLoadingHubDismiss('修改简介成功');
        AppNavigator.goBack(context);
      });
    }).catchError((err) {
      HubView.dismiss();
      HubView.showToastAfterLoadingHubDismiss('修改简介失败:$err');
    });
  }
}
