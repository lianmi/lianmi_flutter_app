...
class Pl5Provider extends ChangeNotifier {
  ...
  void updateCount() {
    ...
    _count = PaiLie5CalculateUtils.calculateMultiple(geCount, shiCount, baiCount, qianCount, wanCount);
    notifyListeners();
  }
  ...
  void selectBallAction() async {
    ...
    bool isCurrentDanshi = (geSelectList.length == 1 && shiSelectList.length == 1 && baiSelectList.length == 1 && qianSelectList.length == 1 && wanSelectList.length == 1);
    if(_selectLotterys.length > 0) {
      if(isCurrentDanshi) {
        if(hasExistDanshi) {
  ...
  void showNumKeep() {
    ...
    AppNavigator.push(
      App.context!, 
      Pl5Page(businessId!, _lotteryId, isNumKeep:true)
    ).then((value) {
