...
import 'package:lianmiapp/pages/lottery/page/lottery_pay_page.dart';
import 'package:lianmiapp/pages/lottery/page/prize_page.dart';
...
import 'package:lianmiapp/pages/lottery/widget/order_detail_list_widget.dart';
...
class _OrderDetailPageState extends State<OrderDetailPage>
  ...
  Widget build(BuildContext context) {
    LotteryProductModel? product =
        LotteryData.instance.getProduct(order.loterryType!);
  ...
  Widget _bottomArea() {
    if (App.isShop) {
      ...
      switch (order.status) {
        ...
        case OrderStateEnum.OS_Confirm:
          {
            if (order.loterryType == 1 || order.loterryType == 2) {
...
//上传我的收款码
  ...
  void _loadImageIfDone() {
    ...
    if (order.status == OrderStateEnum.OS_Prepare ||
      ...
      //获取商家的收款码
      OrderMod.getPayUrl(order.orderID!).then((url) {
        HubView.dismiss();
        if (url == '') {
          // logW('无法获取商家的收款码');
        } else {
          _lotteryUrl = url;
          if (url != '') logI('获取商家的收款码 _lotteryUrl: $_lotteryUrl');
        }
        setState(() {});
      }).catchError((err) {
        HubView.dismiss();
    ...
    // if (order.status == OrderStateEnum.OS_AcceptPrizeed ||
    //     order.status == OrderStateEnum.OS_Prizeed) {
    //   HubView.showLoading();
    //   OrderMod.getPrizeUrl(order.orderID!).then((url) {
    //     HubView.dismiss();
    //     if (url == '') {
    //     } else {
    //       _lotteryUrl = url;
    ...
    FileManager.instance.copyFileToAppFolder(sourceFile).then((value) async {
      ...
      switch (action) {
        ...
        case 3:
          {
            //上传兑奖收款码
            OrderMod.acceptPrize(order.orderID!, sourceFile).then((value) {
              if (value) {
                //TODO 更改订单状态
