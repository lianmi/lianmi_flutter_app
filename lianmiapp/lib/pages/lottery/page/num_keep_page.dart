...
import 'package:lianmiapp/pages/lottery/widget/selected_list_widget.dart';
...
class _NumKeepPageState extends State<NumKeepPage> {
  ...
  void initState() {
    super.initState();
    NotificationCenter.instance.addObserver(NotificationDefine.numKeepUpdate, ({object}) {
      _loadNumKeeps();
    });
    _loadNumKeeps();
  }
  ...
  void dispose() {
    super.dispose();
    NotificationCenter.instance.removeNotification(NotificationDefine.numKeepUpdate);
  }

  void _addNumKeep() {
    int lotteryId = Provider.of<LotteryProvider>(context, listen: false).lotteryId;
    switch (LotteryTypeEnum.values[lotteryId]) {
      case LotteryTypeEnum.ssq: {
        Provider.of<ShuangseqiuProvider>(context, listen: false).showNumKeep();
      }
        break;
  ...
  void _loadNumKeeps() async {
    ...
    switch (LotteryTypeEnum.values[lotteryId]) {
      case LotteryTypeEnum.ssq: {
        _list = await Provider.of<ShuangseqiuProvider>(context, listen: false).getAllNumKeep();
      }
        break;
      case LotteryTypeEnum.fc3d: {
        _list = await Provider.of<Fc3dProvider>(context, listen: false).getAllNumKeep();
      }
        break;
      case LotteryTypeEnum.qlc: {
        _list = await Provider.of<QlcProvider>(context, listen: false).getAllNumKeep();
      }
        break;
      case LotteryTypeEnum.dlt: {
        _list = await Provider.of<DltProvider>(context, listen: false).getAllNumKeep();
      }
        break;
      case LotteryTypeEnum.pl3: {
        _list = await Provider.of<Pl3Provider>(context, listen: false).getAllNumKeep();
      }
        break;
      case LotteryTypeEnum.pl5: {
        _list = await Provider.of<Pl5Provider>(context, listen: false).getAllNumKeep();
      }
        break;
      case LotteryTypeEnum.qxc: {
        _list = await Provider.of<QxcProvider>(context, listen: false).getAllNumKeep();
      }
        break;
      default:
    }
  ...
  void _selectNumKeep(dynamic model) {
    ...
    switch (LotteryTypeEnum.values[lotteryId]) {
      case LotteryTypeEnum.ssq: {
        Provider.of<ShuangseqiuProvider>(context, listen: false).selectNumKeep(model);
      }
        break;
      case LotteryTypeEnum.fc3d: {
        Provider.of<Fc3dProvider>(context, listen: false).selectNumKeep(model);
      }
        break;
      case LotteryTypeEnum.qlc: {
