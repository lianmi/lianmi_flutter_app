...
class ShuangseqiuProvider extends ChangeNotifier {
  ...
  void setup(int id) {
    ...
    if (_blueList.length == 0) {
      for (var i = 0; i < kShuangseqiuBlueCount; i++) {
        _blueList.add(BallNumModel(i + 1));
      }
    }
    if (_redDanList.length == 0) {
      for (var i = 0; i < kShuangseqiuRedCount; i++) {
        _redDanList.add(BallNumModel(i + 1));
      }
    }
    if (_redTuoList.length == 0) {
      for (var i = 0; i < kShuangseqiuRedCount; i++) {
        _redTuoList.add(BallNumModel(i + 1));
      }
    }
    if (_blueDTList.length == 0) {
      for (var i = 0; i < kShuangseqiuBlueCount; i++) {
        _blueDTList.add(BallNumModel(i + 1));
      }
    }
    notifyListeners();
  ...
  void updateCount() {
    if (type == 0) {
      ...
      _count = SSQCalculateUtils.calculateDantuo(
          redDanCount, redTuoCount, blueDTCount);
    }
    notifyListeners();
  }
  ...
  void selectBallAction() async {
    ...
    if (type == 0) {
      ...
      if (_selectLotterys.length > 0) {
        bool isCurrentDanshi =
            (redSelectList.length == 6 && blueSelectList.length == 1);
        if (isCurrentDanshi) {
          if (hasExistDanshi) {
      ...
      int redTuoNeedSelectCount =
          kShuangseqiuRedMinSelectCount - redDanSelectList.length;
  ...
  Future<List<ShuangseqiuModel>> getAllNumKeep() async {
    ...
    List<ShuangseqiuModel> results =
        ShuangseqiuModel.modelListFromDbLotterys(allListFromDb);
    return results;
  }
  ...
  void selectNumKeep(ShuangseqiuModel model) async {
    ...
    if (model.type == 2) {
      ...
      data = {'redBalls': model.redBalls, 'blueBalls': model.blueBalls};
    }
    if (_selectLotterys.length > 0) {
