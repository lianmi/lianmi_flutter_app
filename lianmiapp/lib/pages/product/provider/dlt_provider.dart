...
class DltProvider extends ChangeNotifier {
  ...
  void setup(int id) {
    ...
    if (_backList.length == 0) {
      for (var i = 0; i < kDltBackCount; i++) {
        _backList.add(BallNumModel(i + 1));
      }
    }
    if (_frontDanList.length == 0) {
      for (var i = 0; i < kDltFrontCount; i++) {
        _frontDanList.add(BallNumModel(i + 1));
      }
    }
    if (_frontTuoList.length == 0) {
      for (var i = 0; i < kDltFrontCount; i++) {
        _frontTuoList.add(BallNumModel(i + 1));
      }
    }
    if (_backDTList.length == 0) {
      for (var i = 0; i < kDltBackCount; i++) {
        _backDTList.add(BallNumModel(i + 1));
      }
    }
    notifyListeners();
  ...
  void updateCount() {
    if (type == 0) {
      ...
      _count = DLTCalculateUtils.calculateDantuo(
          frontDanCount, frontTuoCount, backDTCount);
    }
    notifyListeners();
  }
  ...
  Future updateSelectLotterys() async {
    ...
    List dantuoFromDb =
        await AppManager.gRepository!.queryLotterys(_lotteryId, 2, 0);
    if (dantuoFromDb.length > 0) {
      _selectLotterys = DltModel.modelListFromDbLotterys(dantuoFromDb);
    }

    if (danshiFromDb.length == 0 &&
        fushiFromDb.length == 0 &&
        dantuoFromDb.length == 0) _selectLotterys = [];
  ...
  void selectBallAction() async {
    ...
    if (type == 0) {
      ...
      if (_selectLotterys.length > 0) {
        bool isCurrentDanshi = (frontSelectList.length == 5 &&
            backSelectList.length == kDltBackMinSelectCount);
        if (isCurrentDanshi) {
          if (hasExistDanshi) {
      ...
      int backTuoNeedSelectCount =
          kDltFrontMinSelectCount - frontDanSelectList.length;
  ...
  void showNumKeep() {
    ...
    resetAllNum();
    AppNavigator.push(
            App.context!, DltPage(businessId!, _lotteryId, isNumKeep: true))
        .then((value) {
  ...
  Future<List<DltModel>> getAllNumKeep() async {
    ...
    List dantuoFromDb =
        await AppManager.gRepository!.queryLotterys(_lotteryId, 2, 1);
  ...
  void selectNumKeep(DltModel model) async {
    ...
    if (model.type == 2) {
      ...
      data = {'frontBalls': model.frontBalls, 'backBalls': model.backBalls};
    }
    if (_selectLotterys.length > 0) {
