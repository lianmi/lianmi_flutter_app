...
//个位
const int kPl3GeCount = 10;

//十位
const int kPl3ShiCount = 10;

//百位
const int kPl3BaiCount = 10;

class Pl3Provider extends ChangeNotifier {
  int _lotteryId = 0;
  ...
  List<Pl3Model> _selectLotterys = [];

  List<Pl3Model> get selectLotterys => _selectLotterys;
  ...
  void updateCount() {
    ...
    _count =
        PaiLie3CalculateUtils.calculateMultiple(geCount, shiCount, baiCount);
    notifyListeners();
  }
  ...
  Future updateSelectLotterys() async {
    ...
    if (danshiFromDb.length > 0) {
      _selectLotterys = Pl3Model.modelListFromDbLotterys(danshiFromDb);
    }

    List fushiFromDb =
        await AppManager.gRepository!.queryLotterys(_lotteryId, 1, 0);
    if (fushiFromDb.length > 0) {
      _selectLotterys = Pl3Model.modelListFromDbLotterys(fushiFromDb);
    }
    ...
    if (_selectLotterys.length > 0) {
      if (isCurrentDanshi) {
        ...
        if (hasExistDanshi) {
          ...
          for (Pl3Model item in _selectLotterys) {
  ...
  void showNumKeep() {
    ...
    resetAllNum();
    AppNavigator.push(
            App.context!, Pl3Page(businessId!, _lotteryId, isNumKeep: true))
        .then((value) {
  ...
  Future<List<Pl3Model>> getAllNumKeep() async {
    List danshiFromDb =
    ...
    List<Pl3Model> results = Pl3Model.modelListFromDbLotterys(allListFromDb);
    return results;
  }

  void selectNumKeep(Pl3Model model) async {
    AppNavigator.goBack(App.context!);
    Map<String, dynamic> data = {};
    data = {
  ...
  ///根据id删除已选号码
  Future deleteLotterys(List<Pl3Model> list) async {
    for (Pl3Model item in list) {
      await AppManager.gRepository!.deleteLottery(item.id!);
    }
    updateSelectLotterys();
    NotificationCenter.instance
