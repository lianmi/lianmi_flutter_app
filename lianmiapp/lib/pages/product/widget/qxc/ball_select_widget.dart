...
class _BallSelectWidgetState extends State<BallSelectWidget> {
  ...
  Widget build(BuildContext context) {
    return Consumer<QxcProvider>(builder: (context, provider, child) {
      ...
      return Column(
        children: [
          ...
          _ballListArea(provider.oneList, Colours.app_main,
              onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('2位，已选择${twoSelectedCount}个'),
          _ballListArea(provider.twoList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('3位，已选择${threeSelectedCount}个'),
          _ballListArea(provider.threeList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('4位，已选择${fourSelectedCount}个'),
          _ballListArea(provider.fourList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('5位，已选择${fiveSelectedCount}个'),
          _ballListArea(provider.fiveList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('6位，已选择${sixSelectedCount}个'),
          _ballListArea(provider.sixList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('7位，已选择${sevenSelectedCount}个'),
          _ballListArea(provider.sevenList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
        ],
