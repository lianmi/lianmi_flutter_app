...
class _BallSelectWidgetState extends State<BallSelectWidget> {
  ...
  Widget build(BuildContext context) {
    return Consumer<Pl5Provider>(builder: (context, provider, child) {
      ...
      int qianSelectedCount = provider.getSelectedCount(provider.qianList);
      int wanSelectedCount = provider.getSelectedCount(provider.wanList);
      return Column(
        children: [
          _titleArea('个位，已选择${geSelectedCount}个'),
          _ballListArea(provider.geList, Colours.app_main,
              onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('十位，已选择${shiSelectedCount}个'),
          _ballListArea(provider.shiList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('百位，已选择${baiSelectedCount}个'),
          _ballListArea(provider.baiList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('千位，已选择${qianSelectedCount}个'),
          _ballListArea(provider.qianList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
          _titleArea('万位，已选择${wanSelectedCount}个'),
          _ballListArea(provider.wanList, Colours.app_main,
              showUnderLine: false, onTap: (BallNumModel model) {
            _handleBall(model);
          }),
        ],
  ...
  void _handleBall(BallNumModel numModel) {
    ...
    setState(() {});
    Provider.of<Pl5Provider>(App.context!, listen: false).updateCount();
  }
}
